# MS07-Q1｜AlchemyBench 环氧官能共聚物原料与角色预测
...
- 来源方式：AlchemyBench raw_materials 任务直接实例
- Inputs：见 `inputs/` 与 `input_manifest.json`

## Prompt

根据 case.json 预测 RAFT 聚合与后续环氧—硫醇衍生化所需原料，区分单体、链转移剂/引发体系、衍生化试剂、溶剂和辅助剂。不得从标题推断不存在的精确用量。
